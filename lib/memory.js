...
const MEMORY_ORIGINS = new Set(["manual", "summarized", "extracted", "reflected"]);
...
function createMemoryStore(memoryFile) {
  ...
  function addMemory(input) {
    ...
    return withMemoryLock(() => {
      ...
      const origin = normalizeMemoryOrigin(input?.origin, sourceMessageCount);
...
function normalizeMemoryEntry(entry) {
  ...
  const sourceMessageCount = clampInteger(entry.sourceMessageCount, 0, 10_000, 0);

  return {
    id: typeof entry.id === "string" && entry.id.trim() ? entry.id : createId(),
    type: normalizeMemoryType(entry.type),
    origin: normalizeMemoryOrigin(entry.origin, sourceMessageCount),
...
function normalizeMemoryType(value) {
  if (typeof value === "string" && MEMORY_TYPES.has(value.trim().toLowerCase())) {
...
function normalizeMemoryOrigin(value, sourceMessageCount) {
  if (typeof value === "string" && MEMORY_ORIGINS.has(value.trim().toLowerCase())) {
    return value.trim().toLowerCase();
  }

  return sourceMessageCount > 0 ? "summarized" : "manual";
}
